...
if (!function_exists("jr_buzz_refresh")) {
function jr_buzz_refresh() {
update_option("jr_submitted_buzz", "0");
}
}
...
update_option("jr_submitted_buzz", "1");
wp_schedule_single_event(time()+172800, 'jr_buzz_refresh'); 
} else {
...
if (get_option("jr_buzz_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_buzz_link_personal", $content);
}
}
...
$linkper=utf8_decode(get_option('jr_buzz_link_personal'));

if (get_option("jr_buzz_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_buzz_links_choice'));
...
update_option("jr_buzz_links_choice", $new);
update_option("jr_buzz_link_newcheck", "444");
}

if (get_option("jr_submitted_buzz")=="0") {
$pname="jr_buzz";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_buzz", "1");
...
wp_schedule_single_event(time()+172800, 'jr_buzz_refresh'); 
} else if (get_option("jr_submitted_buzz")=="") {
$pname="jr_buzz";
$url=get_bloginfo('url');
$current=get_option("jr_buzz_links_choice");
...
echo "<p style='color: #".$buzzcolor."; font-size:x-small'>Google Buzz Plugin created by ".$linkper." - ".stripslashes(get_option('jr_buzz_links_choice'))."</p>";
}
